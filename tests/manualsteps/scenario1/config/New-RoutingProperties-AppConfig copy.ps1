<#
.SYNOPSIS
Creates configuration for the routing properties in application configuration.
...
Write-Host "Deploying routing properties configuration key $key"

az appconfig kv set --name $configStoreName --key $key --value $value --content-type $type --label $label --tags $tags --yes 

Write-Host "Deployment complete for routing properties configuration key $key"