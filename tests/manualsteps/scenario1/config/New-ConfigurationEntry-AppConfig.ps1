<#
.SYNOPSIS
Creates configuration for the configuration entry in application configuration.
...
Write-Host "Deploying configuration entry configuration key $key"

az appconfig kv set --name $configStoreName --key $key --value $value --content-type $type --label $label --tags $tags --yes 

Write-Host "Deployment complete for configuration entry configuration key $key"